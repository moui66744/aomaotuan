...
<body>
	...
	function passFun(pass){
		...
		        console.log('密码格式不正确：最短6位，最长16位，可包含大小写字母、数字和下划线')
		      return false;
		    } else {
		        console.log('密码格式正确')
		      return true;
	...
	//提交
			$("#submit").click(function(){
				var data = {};
				data.newPassword = $.trim($("input[name=newPassword]").val());
				data.newPassword2 = $.trim($("input[name=newPassword2]").val());
				if (data.newPassword == '') {
					alert("请输入密码");
					return ;
				}
				...
				if(passFun(data.newPassword)==false){
					alert("密码格式不正确：最短6位，最长16位，可包含大小写字母、数字和下划线");
					return;
				}
				$.ajax({
			        url: "ChangePassController.do",
			        async : true,
			        type: "post",
			        dataType: "text",
			        data: data,
				...
				<div class="MeRightArea">
					...
					<div class="informationArea">
						...
						<div class="InforContent">
							...
							<div class="inforArea">
								<!-- <form action="ChangePassController.do"> -->
									<h5>您正在进行<em>修改密码</em>操作，请完成以下步骤</h5>
								<form class="form-horizontal" id="RegistForm" method="post" 
									action="ChangePassController.do"> 
									<div class="inforMail"><span>输入密码：</span><input type="password" name="newPassword"></div>
									<div class="inforMail"><span>确认密码：</span><input type="password" name="newPassword2"></div>
									<div class="inforSubmit"><button id="submit">确定</button></div>
								<!-- </form> -->
								</form> 
							</div>
						</div>
