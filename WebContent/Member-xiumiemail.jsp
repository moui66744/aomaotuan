	...
	<div class="MeMainW">
		<div class="MeMain  w1190">
			<div class="MeArea clearfix">
				...
				<!-- 右边主要内容 -->
				<input id="userEmail" type="hidden" value="${users.userEmail}"/>
<!--  -->
...
$(function(){
	$("#btn").click(function(){	
		if($("#email").val()){
			if(emailFun($("#email").val())){
				if ($("#email").val() == $("#userEmail").val()) {
					$.ajax({
						type:"post",
	...
	$("#btn").click(function(){
		if($("#email").val()){
			$.ajax({
				...
				success:function(data){
					alert("如果您输入的是自己的邮箱，则验证码已经发送成功，请注意查收。");
				}
			})
		}
		else{
			...
			if ($("#email").val()) {
				...
				<div class="MeRightArea">
					...
					<div class="informationArea">
						...
						<div class="inforArea">
							<h5>您正在使用<em>邮箱验证码</em>验证身份，请完成以下操作</h5>							
							<div id="register">
								<form class="form-horizontal" id="RegistForm" method="post"
									action="UserEmailJudgeController.do">
									<fieldset>
										<div class="inforMail" >
											<span>您的邮箱：</span>
											<!-- <div class="col-sm-5"> -->
												<input type="text" placeholder="请填写邮箱地址..." class="form-control"
										...
										<div class="inforText">
											<h6>没收到邮箱验证码？</h6>
											<p>
												1、网络通讯异常可能造成邮件丢失，请重新获取或稍后再试
											</p>
											<p>
												2、请核实邮箱是否正常使用，并检查垃圾邮箱夹
											</p>
											<p>
												3、如果邮箱已停用，请选择其他验证方式
											</p>
