...
<div class="Consigneew boxS w1190">
	<div class="Consignee clearfix">
		...
		<ul class="address clearfix">
		 <c:forEach items="${orderRecipientsList }" var="orderRecipients" varStatus="status"> 
			<li <c:if test="${orderRecipients.recipientsChecked==1}">class="checkbox checked"</c:if>
			<c:if test="${orderRecipients.recipientsChecked==0}">class="checkbox"</c:if>
			 id="${orderRecipients.recipientsId }"><!-- checked -->
				<p class="user">${orderRecipients.recipientsName }</p>
				<p class="site">${orderRecipients.recipientsAddress }</p>
				<p class="phonenum">${orderRecipients.recipientsPhone }</p>
			</li>
		 </c:forEach> 
		</ul>
		<button>
...
<div class="Blurbw boxS w1190">
	...
	<div class="BlurArea">
		...
		<c:forEach items="${shopitemsList }" var="shopitems" varStatus="status">
			<ul class="blur clearfix">
				<li class="first">
				<p>${shopitems.goods.goodsName }</p>
				</li>
				<li>${shopitems.itemNum }</li>
				<li>￥${shopitems.goods.goodsPrice }</li>
				<li>200g</li>
				<li>￥114.0</li>
				<li>￥${shopitems.goods.goodsPrice * shopitems.itemNum}</li>
			</ul>
		</c:forEach>
	...
	<div class="blurFooterw clearfix">
		<div class="blurFooter">
			...
			<p class="submit clearfix">
				<span>返回购物车</span>
				<span>实付总额：￥ <em id="sumPrice">${param.sumPrice}</em></span>  
				<%-- <a href="${pageContext.request.contextPath}/ShopitemToOrdersAndDetailController.do" >提交订单</a> --%>
				<a class="tijiao" style="background:#999" href="javascript:void(0);" >提交订单</a>
...
<div class="mode">
	<div class="modeArea">
		...
		<p class="Rpeople">
			...
			<input class="w170" type="text" placeholder="限20个字" name="recipientsName">
		</p>
		<!-- <p class="Receiving">
			  <strong>
				<em>*</em>
			...
			<select class="w370">
				...
				<option>湖南 长沙市 芙蓉区 文艺路</option>
    									...
    									<div class="left">
										<strong>	<em>*</em>
											<span>所在地区</span></strong>
										</div>
										
										<div class="right" >
											<select name="recipientsAddress1" id="" class="w370">
		...
		<p class="Detailed">
			...
			<input class="w370" type="text" placeholder="无需重复填写省市区，小于50个字"  name="area">
		</p>
		<!-- <p class="IDcard">
			<strong>
				<span>身份证号</span>
		...
		<p class="phonenum">
			...
			<input class="w170" type="text" placeholder="请输入正确的手机号" name="recipientsPhone">
		</p>
		<!-- <p class="Tel">
			<strong>
				<span>固定电话</span>
		...
		<p class="default">
			<strong></strong>
			<span class="checkbox" name="recipientsChecked" id="checkboxmoren" style="margin: 11px"></span>
			<span>设为默认地址</span>
		</p>
		<p class="sure">
			<strong></strong>
			<button onclick="insert()">确定</button>
			<!-- <input type="submit" id="motaiqueding" value="确定" onclick="insert()"/> -->
			<a href="#">取消</a>
		</p>
		<p class="close">+</p>
	</div>
...
<script type="text/javascript">
      ...
      function insert() {
    	  ...
    	  if ($("#checkboxmoren").hasClass('checked')) {
    		  data.recipientsChecked = 1;
    	  }
    	  data.recipientsUser = 1;// ! 后续添加
    	  //console.log(data);
    	  $.ajax({
    		  url: "InsertOrderRecipientsInCartController.do",
    		  type: "post",
    		  data: data,
